...
export const PhoneSchema = z.string().regex(/^\+?[1-9]\d{1,14}$/, 'Teléfono debe tener formato válido');
export const CurrencySchema = z.enum(['USD', 'UYU'] as const);
export const DocumentNumberSchema = z.string().regex(/^\d{7,8}$/, 'Número de documento debe tener 7-8 dígitos');
export const TaxIdSchema = z.string().regex(/^\d{12}$/, 'RUT debe tener 12 dígitos');

// Date validation helpers
export const DateStringSchema = z.string().datetime('Fecha debe ser ISO string válido');
export const DateOnlySchema = z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Fecha debe tener formato YYYY-MM-DD');
...
export const CreateUserSchema = z.object({
  ...
  first_name: z.string().min(1, 'Nombre es requerido').max(100, 'Nombre muy largo'),
  last_name: z.string().min(1, 'Apellido es requerido').max(100, 'Apellido muy largo'),
  phone: PhoneSchema.optional(),
  document_type: z.enum(['cedula', 'passport', 'rut']).optional(),
  document_number: z.string().max(50, 'Número de documento muy largo').optional(),
  address: z.string().max(500, 'Dirección muy larga').optional(),
...
export const CreateProjectSchema = z.object({
  ...
  address: z.string().min(1, 'Dirección es requerida').max(500, 'Dirección muy larga'),
...
export const UpdateProjectSchema = CreateProjectSchema.partial().omit(['organization_id']);
...
export const CreateUnitSchema = z.object({
  ...
  built_area: z.number().positive('Área construida debe ser positiva').optional(),
...
export const UpdateOperationSchema = z.object({
  ...
  notes: z.string().max(1000, 'Notas muy largas').optional(),
  cancellation_reason: z.string().max(500, 'Razón de cancelación muy larga').optional()
});
...
export const CreateDocumentSchema = z.object({
  ...
  description: z.string().max(1000, 'Descripción muy larga').optional(),
  file_name: z.string().min(1, 'Nombre de archivo es requerido').max(255, 'Nombre muy largo'),
...
export const UpdateDocumentSchema = z.object({
  ...
  validation_notes: z.string().max(1000, 'Notas de validación muy largas').optional(),
...
export const CreateProfessionalSchema = z.object({
  ...
  registration_number: z.string().max(100, 'Número de registro muy largo').optional(),
  specializations: z.array(z.string()).default([]),
  service_areas: z.array(z.string()).default([]),
  hourly_rate: z.number().positive('Tarifa debe ser positiva').optional()
});

export const UpdateProfessionalSchema = CreateProfessionalSchema.partial().omit(['user_id']);
...
export const PaginationSchema = z.object({
  ...
  pageSize: z.number().int().positive('Tamaño de página debe ser positivo').min(1).max(100).default(20)
});
...
export const CreateDataCorrectionSchema = z.object({
  ...
  correction_reason: z.string().min(1, 'Razón de corrección es requerida').max(1000, 'Razón muy larga'),
...
export const FileUploadSchema = z.object({
  ...
  allowed_types: z.array(z.string()).default(['application/pdf', 'image/jpeg', 'image/png'])
});
