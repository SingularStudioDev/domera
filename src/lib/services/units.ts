...
export async function bulkCreateUnitsWithValidation(
  ...
  try {
    ...
    if (!validationResult.success) {
      const errors: BulkUnitValidationError[] =
        validationResult.error.issues.map((issue, index) => ({
          index: issue.path.length > 1 ? parseInt(issue.path[1] as string) : 0,
          unitNumber:
            issue.path.length > 2 && typeof issue.path[2] === "string"
              ? units[parseInt(issue.path[1] as string)]?.unit_number
              : undefined,
      ...
      return failure(
        `Errores de validación en ${errors.length} unidades: ${errors.map((e) => e.message).join(", ")}`,
      );
    }
    ...
    if (businessValidationErrors.length > 0) {
      return failure(
        `Errores de reglas de negocio en ${businessValidationErrors.length} unidades`,
      );
    }
...
export async function validateUnitNumbersAvailable(
  ...
  try {
    ...
    if (existingCount > 0) {
      return failure(
        `${existingCount} números de unidad ya existen en el proyecto`,
      );
    }
...
> {
  try {
    ...
    const recentBatches = Object.entries(batches).map(
      ([dateStr, unitNumbers]) => ({
