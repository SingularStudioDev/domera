    ...
    <form onSubmit={handleCredentialsSubmit} className="w-full space-y-4">
      <div>
        ...
        <input
          ...
          className="focus:ring-blue-500 w-full rounded-md border border-gray-300 px-4 py-3 transition-all outline-none focus:border-transparent focus:ring-2"
      ...
      <div>
        ...
        <div className="relative">
          <input
            ...
            onChange={handleInputChange}
            className="focus:ring-blue-500 w-full rounded-md border border-gray-300 px-4 py-3 pr-12 transition-all outline-none focus:border-transparent focus:ring-2"
    ...
    <div className="w-full space-y-4">
      <div className="text-center mb-6">
        ...
        <p className="text-sm text-gray-600">
          Hemos enviado un código de 6 dígitos a <strong>{userData?.email}</strong>
        </p>
        {countdown > 0 && (
      ...
      <form onSubmit={handleVerificationSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            className="focus:ring-blue-500 w-full rounded-md border border-gray-300 px-4 py-3 text-center text-2xl tracking-widest transition-all outline-none focus:border-transparent focus:ring-2"
            placeholder="000000"
            maxLength={6}
            required
            disabled={isSubmitting}
      ...
      <div className="text-center space-y-2">
        ...
        >
          {countdown > 540 ? 'Espera 1 minuto para reenviar' : 'Reenviar código'}
        </button>
          ...
          <div className="mx-auto flex w-full max-w-md flex-col items-center justify-center">
            ...
            <div className="w-full mb-8">
              <div className="flex items-center justify-center space-x-4 mb-4">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium ${
                  authStep === 'credentials' ? 'bg-blue-600 text-white' : 
                  authStep === 'verification' || authStep === 'success' ? 'bg-green-600 text-white' : 
                  'bg-gray-200 text-gray-600'
                }`}>
                  1
                </div>
                <div className={`h-1 w-16 rounded ${
                  authStep === 'verification' || authStep === 'success' ? 'bg-green-600' : 'bg-gray-200'
                }`}></div>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium ${
                  authStep === 'verification' ? 'bg-blue-600 text-white' : 
                  authStep === 'success' ? 'bg-green-600 text-white' : 
                  'bg-gray-200 text-gray-600'
                }`}>
                  2
                </div>
                ...
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium ${
                  authStep === 'success' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-600'
                }`}>
                  ✓
                </div>
              </div>
            ...
            <div className="mb-8 text-center">
              ...
              <p className="text-gray-600">
                {authStep === 'credentials' && 'Acceso exclusivo para super administradores de Domera'}
                {authStep === 'verification' && 'Verifica tu identidad con el código enviado'}
                {authStep === 'success' && 'Autenticación completada exitosamente'}
              </p>
            </div>
            ...
            {error && (
              <div className={`mb-4 rounded-md border p-3 ${
                error.includes('enviado') ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'
              }`}>
                <p className={`text-sm ${
